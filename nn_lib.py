...
class SigmoidLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        forward_output = self._cache_current

        sigmoid_grad = forward_output * (1 - forward_output)

        grad_input = grad_z * sigmoid_grad
...
            - neurons {list} -- Number of neurons in each linear layer 
        ...
        for layer_dim_i, acti_i in zip(self.neurons, self.activations):
            self._layers.append(LinearLayer(layer_input_dim, layer_dim_i))
            if acti_i == "relu":
                self._layers.append(ReluLayer())
            elif acti_i == "sigmoid":
                self._layers.append(SigmoidLayer())
            elif acti_i == "identity":
                self._layers.append(IdentityLayer())
            else:
                raise AssertionError("Unsupported activation function!")
            layer_input_dim = layer_dim_i
        #######################################################################
        #                       ** END OF YOUR CODE **
        #######################################################################
    ...
    def update_params(self, learning_rate):
        ...
        #######################################################################
        for layer_i in self._layers:
            if hasattr(layer_i, "update_params"):
...
class Trainer(object):
    ...
    def __init__(
        ...
        if self.loss_fun == "mse":
            ...
            raise AssertionError("Unsupported loss function!")
        #######################################################################
        #                       ** END OF YOUR CODE **
        #######################################################################
    ...
    def train(self, input_dataset, target_dataset):
        ...
        assert np.shape(input_dataset)[0] == np.shape(target_dataset)[0]

        for epoch_i in range(self.nb_epoch):
            if self.shuffle_flag == True:
                input_dataset, target_dataset = self.shuffle(input_dataset, target_dataset)

            num_training_data_points = np.shape(input_dataset)[0]
            
            epoch_loss = 0
            for idx in range(0, num_training_data_points, self.batch_size):
                x_batch = input_dataset[idx:min(idx+self.batch_size, num_training_data_points)]
                y_batch = target_dataset[idx:min(idx+self.batch_size, num_training_data_points)]
            ...
            logging.info(f"Epoch {epoch_i+1}/{self.nb_epoch}: Loss {epoch_loss:.4f}")
...
class Preprocessor(object):
    ...
    def apply(self, data):
        ...
        # assert (self.min == np.min(data, axis=0)).all() and (self.max == np.max(data, axis=0)).all()
        return (data - self.min) / (self.max - self.min + 1e-10)

        # return (data - np.min(data, axis=0)) / (np.max(data, axis=0) - np.min(data, axis=0) + 1e-10)
    ...
    def revert(self, data):
        ...
        #######################################################################
        # assert (self.min == np.min(data, axis=0)).all() and (self.max == np.max(data, axis=0)).all()
        return data * (self.max - self.min + 1e-10) + self.min
    
        # return data * (np.max(data, axis=0) - np.min(data, axis=0) + 1e-10) + np.min(data, axis=0)
